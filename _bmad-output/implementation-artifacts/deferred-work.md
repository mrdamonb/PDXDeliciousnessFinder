...
- **`VisitLogDTO.toModel()` never hydrates the `restaurant` relationship** (`Core/Network/DTOs/VisitLogDTO.swift:66`). Only `AddVisitView.swift:66` sets `visitLog.restaurant`. Every visit arriving through `pullFromRemote` or a realtime insert is an orphan, and `HistoryView` gates each row on `if let restaurant = log.restaurant`, so those visits never render in History at all. After an Xcode reinstall — which the Sprint 1 checklist treats as routine — the local store repopulates entirely from remote and **every** visit becomes an orphan. This is the root cause behind story 3.5's bare-month-header finding; the fix applied there only suppresses the empty section, it does not make synced visits visible. Deserves its own story.

...
2a. **Shipped 2026-09-13 as `c07f22f`, pushed to `main`.** Merged 64px bar (badge, pill search with active-query tint, 44px avatar, + still in the bar), floating Map/List/Journal text pill, Filter chip on Map/List, "N visits" chip on Journal (reported up by `HistoryView`), opaque 60px pill band + 12px fade on List/Journal, `ListView` sort row now scrolls with the rows, Journal sticky headers offset to the band. Constants: `BAR_HEIGHT` 64, `PILL_BAND_HEIGHT` 60. Verified in the real component via a throwaway preview route with sample data, at 390 and 320px: Map, List at rest and scrolled, Filter popover, 10 active filters at 320. Pill segment padding cut to 12px so the two-digit Filter chip fits at 320.
   **Not verified:** Journal rows, sticky headers, and the visit-count chip (the preview had no session, so the Journal showed its load-error state).
   **Known until 2b:** at 320px the search field is narrow ("Search you…") because + is still in the bar; 2b's move to a FAB gives it back 44px. The Journal placeholder "Search restaurants and notes…" fits at 390px but truncates at 320px even after 2b.
   **Bug found on Damon's phone after 2a shipped (2026-09-13):** on iOS Safari the Journal month header parked about 60px low, over the first entry. The scroll container had `padding-top: 60px` and the sticky headers `top: 60px`; Safari applied the padding to the sticky offset a second time (measured from Damon's screenshot: header at about bar + 60 + 60). Fixed by replacing the padding with a 60px spacer element. Chrome never showed it; the fix is not yet verified on iOS.
2b. **Built 2026-09-13, not yet committed.** + leaves the bar and becomes a 56px FAB on Map and List (bottom-right). On Map it sits at 150px, above the resting pill and clear of Google's camera control (first tried Design's 116px; it covered the control). Hidden while a restaurant panel is open and on Journal. The Journal's own add-a-visit FAB now matches: 56px, right 16, same shadow. Verified in the real component at 320px: search field shows its full placeholder, List FAB, FAB hides when a pin's panel opens, map FAB clear of the camera control.
   Originally planned scope: merged bar, floating pill, FAB, Filter chip on Map and List, pill-clearance padding and opaque band on List and Journal, Journal sticky-header offset, FAB hidden while the restaurant panel is up.
